...
// TODO: require return statements in if/else blocks
...
func (p *Parser) parseExpressionList(stopToken token.TokenType) []ast.Expression {
	...
	for p.peekTokenIs(token.COMMA) { // Comma denotes another expression in the list
		p.nextToken()
		p.nextToken()
		list = append(list, p.parseExpression(LOWEST))
	}
...
func (p *Parser) noPrefixParseFnError(t token.TokenType) {
	msg := fmt.Sprintf("no prefix parse function exists for %s", t)
	p.errors = append(p.errors, msg)
}
...
func (p *Parser) peekError(tokenType token.TokenType) {
	msg := fmt.Sprintf("expected next token to be %s, got %s", tokenType, p.peekToken.Type)
	p.errors = append(p.errors, msg)
}
